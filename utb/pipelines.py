...
class UtbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('utb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `utb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM utb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO utb (date, title, link, content)"
